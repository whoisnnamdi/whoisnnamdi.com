...
function getBadge(post) {
    ...
    if (!tag) return 'AI'
    return tag.length <= 3 ? tag.toUpperCase() : tag.slice(0, 2).toUpperCase()
}
        ...
        return (
            <Link href={`/${post.slug}`} className="block group">
                <article className="grid grid-cols-[84px,1fr,64px] gap-6 items-center py-6 border-b border-neutral-200">
                    ...
                    <div className="h-12 w-12 border border-neutral-300 bg-white text-neutral-700 flex items-center justify-center text-[11px] font-mono uppercase tracking-[0.2em]">
                        {getBadge(post)}
                    </div>
                </article>
            </Link>
