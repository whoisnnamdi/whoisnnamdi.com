...
function parseFromH2Sections(html) {
  const items = []
...
function parseFromBookmarks(html) {
  const items = []
  const containers = [...html.matchAll(/<a[^>]*class=\"[^\"]*kg-bookmark-container[^\"]*\"[^>]*href=\"([^\"]+)\"[^>]*>([\s\S]*?)<\/a>/gi)]
  for (const m of containers) {
    const href = m[1]
    const inner = m[2]
    const titleMatch = /<div[^>]*class=\"[^\"]*kg-bookmark-title[^\"]*\"[^>]*>([\s\S]*?)<\/div>/i.exec(inner)
    const descMatch = /<div[^>]*class=\"[^\"]*kg-bookmark-description[^\"]*\"[^>]*>([\s\S]*?)<\/div>/i.exec(inner)
    const imgMatch = /<div[^>]*class=\"[^\"]*kg-bookmark-thumbnail[^\"]*\"[^>]*>[\s\S]*?<img[^>]*src=\"([^\"]+)\"[^>]*>/i.exec(inner)
    const title = titleMatch ? decode(titleMatch[1].replace(/<[^>]*>/g, '').trim()) : ''
    const excerpt = descMatch ? decode(descMatch[1].replace(/<[^>]*>/g, '').trim()) : ''
